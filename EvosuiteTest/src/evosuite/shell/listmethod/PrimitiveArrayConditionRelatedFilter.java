...
public class PrimitiveArrayConditionRelatedFilter extends MethodFlagCondFilter {
	public static final String excelProfileSubfix = "_primArrayConditionRelatedProfiles.xlsx";
	private static Logger log = LoggerUtils.getLogger(PrimitiveArrayConditionRelatedFilter.class);
	private ExcelWriter writer;

	public PrimitiveArrayConditionRelatedFilter() {
...
//				0);
	...
	protected boolean checkMethod(ClassLoader classLoader, String className, String methodName, MethodNode node,
		...
		for (BytecodeInstruction insn : getIfBranchesInMethod(cfg)) {
			if (checkBranchDependentOnPrimitiveArray(classLoader, className, methodName, node, insn)) {
				return true;
			}
		}
		return false;
	...
	private boolean checkBranchDependentOnPrimitiveArray(ClassLoader classLoader, String className, String methodName,
			...
			if (val instanceof SourceValue) {
				...
				for (AbstractInsnNode sourceIns : srcValue.insns) {
					...
					if (src.getASMNode().getOpcode() == Opcodes.GETSTATIC
						...
						if (!desc.equals("[Ljava/lang/String;") && !desc.equals("[Z") && !desc.equals("[B") 
								&& !desc.equals("[C") && !desc.equals("[S") && !desc.equals("[I") && !desc.equals("[J")
										&& !desc.equals("[S") && !desc.equals("[D")) {
							continue;
						} else {
							System.currentTimeMillis();
						}
						...
						if (isParameter(src)) {
							...
							if (name.equals("java.lang.String[]") || name.equals("boolean[]") || name.equals("byte[]")
									|| name.equals("char[]") || name.equals("short[]") || name.equals("int[]")
