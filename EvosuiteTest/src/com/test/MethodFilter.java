...
//				if (!"deleteRepository(Ljava/lang/String;)V".equals(methodName)) {
//					continue;
//				}
			...
			if (CollectionUtils.existIn(insnNode .getOpcode(), Opcodes.IFEQ, Opcodes.IFNE)) {
				...
				if (value instanceof SourceValue) {
					...
					if (isInvokeMethodInsn(condDefinition)) {
						System.out.println("!FOUND IT! in method " + methodName);
						return true;
					} else {
						BytecodeInstruction condBcDef = cfg.getInstruction(node.instructions.indexOf(condDefinition));
						if (condBcDef.isUse()) {
							if (!defuseAnalyzed) {
								DefUseInstrumentation instr = new DefUseInstrumentation();
							...
							if (isInvokeMethodInsn(lastDef.getASMNode())) {
								System.out.println("!FOUND IT! in method " + methodName);
								return true;
							}
						}
