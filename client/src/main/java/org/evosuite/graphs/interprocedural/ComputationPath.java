...
public class ComputationPath {
	...
	public ComputationPath(Branch b) {
		this.branch = b;
	}
...
//		return isConstant;
	...
	public static List<ComputationPath> computePath(DepVariable root, List<BytecodeInstruction> operands, Branch b) {
		List<ComputationPath> computationPath = new ArrayList<>();
		List<DepVariable> nodes = new ArrayList<>();
		// traverse input to oprands
		nodes.add(root);
		dfsRoot(root, operands, computationPath, nodes, b);
		return computationPath;
	}

	private static void dfsRoot(DepVariable root, List<BytecodeInstruction> operands,
			List<ComputationPath> computationPath, List<DepVariable> nodes, Branch b) {
...
//						&& operands.contains(node.getInstruction().getLineNumber())
					...
					nodes.add(node);
					dfsRoot(node, operands, computationPath, nodes, b);
				} else {
					nodes.add(node);
					break;
				}
		...
		if (operands.contains(nodes.get(nodes.size() - 1).getInstruction())) {
			...
			ComputationPath pathRecord = new ComputationPath(b);
