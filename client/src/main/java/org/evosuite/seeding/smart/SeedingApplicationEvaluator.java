...
//						Class<?> cla = findOpcodeType(path);
						...
						if (otherPath != null && otherPath.isConstant()) {
...
//								cla = findOpcodeType(otherPath);
	...
	private static Branch compileBranch(Map<Branch, Set<DepVariable>> branchesInTargetMethod, Branch b) {
		...
		for (Branch br : branchesInTargetMethod.keySet()) {
			...
			String info = s[3] + " " + s[4];
			if (br.toString().contains(info) && br != b) {
				targetBranch = br;
				break;
			}
		}
	...
	private static List<ComputationPath> removeRedundancyPath(List<ComputationPath> pathList) {
		...
		for (int i = 0; i < pathList.size(); i++) {
			...
			for (int j = i + 1; j < pathList.size(); j++) {
				...
				if (!path.getComputationNodes().get(0).isConstant() && size >= 2 && sizeNext >= 2 && path
						.getComputationNodes().get(size - 2) == pathNext.getComputationNodes().get(sizeNext - 2)) {
			...
			if (path.getComputationNodes().get(0).isConstant() && haveLineConstant) {
				if (path.getComputationNodes().get(0).getLineNumber() != path.getComputationNodes().get(size - 1)
						.getLineNumber()) {
